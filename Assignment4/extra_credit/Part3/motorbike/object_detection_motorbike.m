...
detecter_height = 80;
result_display = 1;
load train_matrix_motorbike_50.mat;
labels = [];
predicted = [];
fid = fopen('motorbike_trainval.txt');
alpha = 2;
beta = 1.2;
...
for j=1:Images_to_detect
 ...
 for r = 1:R %lowest to highest
  ...
  scale = alpha^(R-r);
  width1 = detecter_size(r,1);   %% Detecter size fixed for all the scales
  ...
  Y{r}{s} = zeros(floor(orig_height/scale_res), floor(orig_width/(scale_res)));
   ...
   if(result_display)
      %if(locations > 0)
       I = imread(image);
I = imresize(I, [80+32+32 120+32+32]);
       [h w c] = size(I);
	...
	%fprintf(fid_windows,'%s \n \n', tline);
	 ...
	 imshow(I);
	   ...
	   for col = 1:c1
	   if(Y{R}{s}(row,col) == 1)
             rectangle('Position',[floor(col*beta^(S-s)) floor(row*beta^(S-s)) floor(detecter_size(R,1)*beta^(S-s))  floor(detecter_size(R,2)*beta^(S-s))], 'LineWidth',0.5, 'EdgeColor','b');
	     %fprintf(fid_windows,'%s %20f %12f %12f %12f\n', tline, floor(col*beta^(S-s)),floor(row*beta^(S-s)),floor(detecter_size(R,1)*beta^(S-s)),  floor(detecter_size(R,2)*beta^(S-s)));
